  ...
  return (
    <div className="space-y-5">
      <div
        {...getRootProps()}
        className={cn(
          'relative cursor-pointer overflow-hidden rounded-2xl border border-dashed bg-white/80 p-8 text-center shadow-soft transition sm:p-12',
          isDragActive
            ? 'border-accent bg-accent-soft/40'
            : 'border-line hover:border-accent/40 hover:bg-white',
          images.length >= maxImages && 'cursor-not-allowed opacity-70',
        )}
      >
        <input {...getInputProps()} />
        <motion.div
        ...
        {images.length > 0 ? (
          ...
          >
            {images.map((image, index) => (
              <ImageCard key={image.id} image={image} onRemove={removeImage} index={index} />
            ))}
          </motion.div>
        ) : null}
      </AnimatePresence>
